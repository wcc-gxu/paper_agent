...
WebSocket 连接建立后，Server **必须**先发 `connected` 事件，iOS 收到后才能开始发 `chat`：

```
iOS → Server:  WS 连接 /ws/chat/agent-001/main
Server → iOS:  connected 事件（含 session 元数据）
iOS → Server:  chat 消息
```

```json
// Server → iOS（连接建立后立即发送）
{
  "role": "assistant",
  "type": "connected",
  "agentId": "agent-001",
...
**Server 自动创建。** iOS 不需要先调 API。

```
iOS → WS /ws/chat/agent-001/session-not-exist

Server:
  1. 查 agent-001 的 sessions 表 → 不存在 session-not-exist
  2. 自动创建 session:
     session_id = "session-not-exist"
     title = "新对话"       ← 默认占位，第一条 chat 后 LLM 更新
  3. 初始化该 session 的 LangGraph checkpoint (空状态，冷启动)
  4. 发送 connected 事件
  5. iOS 正常开始聊天
```
...
> iOS 必须在收到 `connected` 后才能发 `chat`。用于确认连接就绪、session 有效、获取当前状态。
...
Server → iOS:  connected({agentName:"我的科研助理", sessionTitle:"Transformer文献调研", ...})

# 2. 用户发消息
iOS → Server:
  {"role":"user","type":"chat","sessionId":"sess-abc123","priority":1,"payload":{"content":"搜 Transformer 注意力机制论文"}}
...
> 版本: v2.3 | 连接握手(connected)、自动创建session、session_updated标题通知、默认值写死 | 2026-06-16
