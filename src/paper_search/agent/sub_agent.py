...
编排 IngestAgent 的 7 阶段论文入库流水线:
  1. search_stage    → Engine.search()            [直接]
  2. evaluate_stage  → LLM.evaluate_batch()       [直接]
  3. download_stage  → Celery download_task        [Celery]
  4. convert_stage   → Celery convert_task         [Celery]
  5. index_stage     → Celery index_task           [Celery]
  6. rank_stage      → JournalRanker.rank_batch()  [直接]
  7. survey_stage    → Celery survey_task          [Celery]
