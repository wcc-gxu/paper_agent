...
│
├── session: "main"           ← 默认会话，长期记忆的入口
│   └── "Transformer 注意力调研..."
│
├── session: "cv-project"     ← 独立会话，Transformer 上下文不混入 CV
│   └── "YOLO 目标检测..."
│
...
iOS 上创建"新对话" → POST `/api/agents/{agent_id}/sessions` → 获得新 `session_id` → 连接 `ws://.../agent-001/session-new`。切换已有对话 → 使用该对话的 `session_id` 重连。
...
> 版本: v2.2 | agent_id+session_id 双层、Session内存隔离、Manifest启动协议、API端点、自动标题 | 2026-06-16
